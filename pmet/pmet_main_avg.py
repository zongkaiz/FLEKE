...
def execute_pmet(
    ...
    for requests in split_requests:
        ...
        zs_clients.append(z_list) # 存储每个客户端的 zs
    # Insert
    for requests, z_list in zip(split_requests, zs_clients):
    ...
    #    )
    # 使用该函数
    averaged_deltas = average_deltas(all_deltas)
    return averaged_deltas

# 假设 all_deltas 是你从模型计算过程中得到的包含各层 delta 的列表
def average_deltas(all_deltas):
    # 取得所有的 deltas 的键，通常是层的名称
    delta_keys = all_deltas[0].keys()
    
    # 初始化一个空的字典用于存储每个层的平均值
    averaged_deltas = {}
    
    # 对每个键（即每一层的 delta）进行平均计算
    for key in delta_keys:
        # 对每个层的 delta 求平均
        delta_list = [deltas[key] for deltas in all_deltas]
        # 将所有 deltas 堆叠起来并计算平均值
        averaged_deltas[key] = torch.mean(torch.stack(delta_list), dim=0)
