...
RESULTS_DIR = Path("/media/h3c/users/zongkai/MEMIT/rewriting-konwledge/results")
KV_DIR = Path("/media/h3c/users/zongkai/MEMIT/rewriting-konwledge/kvs")

#定义客户端数据的路径和客户端名称列表。
CLIENTS_DIR = Path("client_z")###
NUM_CLIENTS = 8###
CLIENT_NAMES = [f"client{i}" for i in range(1, 9)]###
def adjust_client_vectors(client_vectors, NUM_CLIENTS, T=4, TARGET_VECTOR_COUNT=10000):
    """
    调整客户端向量的数量，确保选中的 NUM_CLIENTS 个客户端的总向量数为 TARGET_VECTOR_COUNT。
    通过从剩余客户端中按比例分配向量，或者根据当前向量数做减少调整。

    Args:
        client_vectors (dict): 所有客户端的向量数据，结构为 {客户端名称: {时刻: {case_id: v_star}}}
        NUM_CLIENTS (int): 客户端数量，决定选择前多少个客户端。
        T (int): 时刻数，默认值为 4。
        TARGET_VECTOR_COUNT (int): 目标总向量数，默认值为 10000。

    Returns:
        dict: 调整后的客户端向量数据。
    """
    # 取前 NUM_CLIENTS 个客户端
    selected_clients = list(client_vectors.keys())[:NUM_CLIENTS]
    
    # 计算当前选中的客户端的总向量数
    current_vector_count = sum(
        len(client_vectors[client][t]) for client in selected_clients for t in range(T)
    )
    
    if current_vector_count == TARGET_VECTOR_COUNT:
        print(f"客户端总向量数已经是 {TARGET_VECTOR_COUNT}，无需调整。")
        return client_vectors
    
    if current_vector_count < TARGET_VECTOR_COUNT:
        # 如果当前向量数小于 TARGET_VECTOR_COUNT，从后面的客户端中按比例分配向量
        remaining_clients = list(client_vectors.keys())[NUM_CLIENTS:]  # 后面的客户端
        
        # 收集剩余客户端的向量
        vectors_to_distribute = {t: [] for t in range(T)}  # 初始化时刻字典
        
        for client in remaining_clients:
            for t in range(T):
                vectors_to_distribute[t].extend(client_vectors[client][t].items())  # 保证是 (case_id, v_star) 键值对
        
        # 计算需要分配的向量数
        remaining_vectors = TARGET_VECTOR_COUNT - current_vector_count
        
        # 计算每个客户端应分配的向量数
        vectors_per_client_per_t = remaining_vectors // (NUM_CLIENTS * T)
        remainder = remaining_vectors % NUM_CLIENTS
        
        # 平均分配向量
        for i, client in enumerate(selected_clients):
            for t in range(T):
                start_idx = i * vectors_per_client_per_t
                end_idx = start_idx + vectors_per_client_per_t + (1 if i < remainder else 0)
                client_vectors[client][t].update(dict(vectors_to_distribute[t][start_idx:end_idx]))
                
        print(f"已将剩余向量分配，前 {NUM_CLIENTS} 客户端总向量数达 {TARGET_VECTOR_COUNT}。")
        
    else:
        # 如果总向量数大于 10000，需要平均减少每个客户端的向量数量
        excess_vector_count = current_vector_count - TARGET_VECTOR_COUNT
        excess_per_client = excess_vector_count // (NUM_CLIENTS * T)
        remainder = excess_vector_count % (NUM_CLIENTS * T)
        
        for client in selected_clients:
            for t in range(T):
                # 每个时刻去除一定数量的向量
                vectors = client_vectors[client][t]
                reduced_count = excess_per_client
                if remainder > 0:
                    reduced_count += 1
                    remainder -= 1
                
                # 从该时刻的向量中移除部分向量
                client_vectors[client][t] = dict(list(vectors.items())[:-reduced_count])
        
        print(f"已减少过多的向量，前 {NUM_CLIENTS} 客户端总向量数达 {TARGET_VECTOR_COUNT}。")
    
    # 打印每个客户端在每个时刻字典中的数量
    total_count = 0
    for client in selected_clients:
        print(f"客户端 {client} 的向量字典数量：")
        for t in range(T):
            print(f"  时刻 {t}: {len(client_vectors[client][t])} 个元素")
            total_count += len(client_vectors[client][t])
...
def main(
    ...
    print(f"执行 {alg_name} 使用参数 {hparams}")
    ...
    # 加载数据
    print("加载数据集、属性摘要、tf-idf 数据")
    snips = AttributeSnippets(DATA_DIR) if not skip_generation_tests else None
    vec = get_tfidf_vectorizer(DATA_DIR) if not skip_generation_tests else None
    gen_test_vars = [snips, vec]
    if num_edits > 1:
        assert ds_name != "cf", f"{ds_name} 不支持多次编辑"
    ...
    if use_cache:
        ...
        print(f"将从 {cache_template} 加载缓存")
    ...
    print("已加载所有客户端的向量。")
    # 假设在 main 中调用
    client_vectors = adjust_client_vectors(client_vectors, NUM_CLIENTS, T)
    ## 对每个客户端按内层字典的键值对中的键（case_id）进行排序
    #for client in client_vectors.keys():
    #    # 将内层字典的键（0, 1, ...）按照其值的键（如 10605, 7, ...）排序
    #    sorted_items = sorted(
    #        client_vectors[client].items(),  # 提取字典中的键值对
    #        key=lambda item: next(iter(item[1].keys()))  # 根据内层字典的键排序
    #    )
    #    # 转回字典并覆盖
    #    client_vectors[client] = dict(sorted_items)
    ## 遍历 client_vectors 中所有客户端
    #for client in client_vectors.keys():
    #    # 获取当前客户端对应字典的所有值，保持原顺序
    #    original_values = list(client_vectors[client].values())
    #    
    #    # 重新构造新的字典，键从 0 开始递增，值保持原顺序
    #    reordered_dict = {i: original_values[i] for i in range(len(original_values))}
    #    
    #    # 更新当前客户端的值为新的字典
    #    client_vectors[client] = reordered_dict
    # 遍历每个客户端
    for client in CLIENT_NAMES:
        print(f"正在处理 {client}")
        ...
        for t in range(T):
            ...
            edited_model, weights_copy = apply_algo(
                ...
                **({"return_orig_weights_device": "cpu"} if conserve_memory else {}),
                **({"cache_template": cache_template} if alg_name in ["ROME", "MEMIT"] else {}),
            )
            if t==0:
...
def chunks(arr, n):#这是一种生成器函数，它会在需要时逐个生成分块，而不是一次性返回所有分块，从而节省内存。
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="MEMIT",
        help="使用的编辑算法。结果保存在 results/<alg_name>/<run_id>，每次运行会生成新的 run_id。"
             "如果从之前的运行继续，请在 --continue_from_run 中指定 run_id。",
    )
    parser.add_argument(
        "--model_name",
    ...
    parser.add_argument(
        ...
        help="超参数文件的名称，位于 hparams/<alg_name> 文件夹中。",
    )
    parser.add_argument(
        "--ds_name",
        choices=["mcf", "cf", "zsre"],
        default="zsre",
        help="要进行评估的数据集。可以是 CounterFact (cf)、MultiCounterFact (mcf) 或 zsRE (zsre)。",
    )
    parser.add_argument(
        "--continue_from_run",
        type=str,
        default=None,
        help="如果从之前的运行继续，请设置为 run_id。否则，保持为 None。",
    )
    parser.add_argument(
        "--dataset_size_limit",
        type=int,
        default=None,
        help="将 CounterFact 数据集截断为前 n 条记录。",
    )
    parser.add_argument(
        "--skip_generation_tests",
        dest="skip_generation_tests",
        action="store_true",
        help="仅运行快速的基于概率的测试，而不进行慢速的生成测试。"
             "适用于快速调试和超参数搜索。",
    )
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="每 [flag_value] 次迭代执行一次生成测试。如果设置为 -1，则跳过生成测试。",
    )
    parser.add_argument(
        "--conserve_memory",
        dest="conserve_memory",
        action="store_true",
        help="在评估过程中减少内存使用，代价是略微的速度下降。"
             "将模型权重备份到 CPU 而不是 GPU。",
    )
    parser.add_argument(
        "--num_edits",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_false",
        default=True,
        help="使用缓存的 k/v 对（默认：True，使用 --use_cache 设置为 False）",
    )
    parser.add_argument(
        "--T",
        type=int,
